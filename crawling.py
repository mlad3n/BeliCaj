...
import dataaccess
from parse_url import parse_links
        ...
        if response.status == 200:
            ...
            if content_type in ('text/html', 'application/xml'):
                ...
                if state != "Post":
                    ...
                    urls = set([urljoin(str(response.url), str(a['href'])) for a in urls])

                    if urls:
                        LOGGER.info('got %r distinct urls from %r',
                                    len(urls), response.url)

                    title_urls = title_urls.union(urls)

                    next_link = soup.find("span", {"class": "next-button"})
                    next_link = next_link.find('a', href=True)
                    ...
                    key = {"date": value['date'], "url": str(response.url)}
